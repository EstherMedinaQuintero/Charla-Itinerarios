...
export class Star extends Figure {
     ...
     */
    constructor(points, outerRadius, innerRadius, position, style, canvasWrapper) {
    ...
    draw() {
        ...
        CONTEXT.beginPath();
        CONTEXT.moveTo(this.position.xPosition + this.outerRadius * Math.cos(0), this.position.yPosition + this.outerRadius * Math.sin(0));
        for (let i = 0; i < 2 * this.points; i++) {
            const RADIUS = i % 2 === 0 ? this.outerRadius : this.innerRadius;
            const ANGLE = angleIncrement * i;
            CONTEXT.lineTo(this.position.xPosition + RADIUS * Math.cos(ANGLE), this.position.yPosition + RADIUS * Math.sin(ANGLE));
        }
        CONTEXT.closePath();
        CONTEXT.fill();
        CONTEXT.stroke();
